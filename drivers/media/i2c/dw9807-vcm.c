...
static int dw9807_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl->id == V4L2_CID_FOCUS_ABSOLUTE) {
		...
		move_pos = abs((int)dev_vcm->current_val - ctrl->val);
		dev_vcm->first = false;
		dev_vcm->current_val = ctrl->val;

		ret = dw9807_ramp(client, start, ctrl->val);
		if (ret)
			return ret;
...
static long dw9807_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct dw9807_device *dev_vcm = sd_to_dw9807_vcm(sd);
...
#ifdef CONFIG_COMPAT
static long dw9807_compat_ioctl32(struct v4l2_subdev *sd,
				   unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == RK_VIDIOC_COMPAT_VCM_TIMEINFO) {
		...
		ret = dw9807_ioctl(sd, RK_VIDIOC_VCM_TIMEINFO, &vcm_tim);
...
static const struct v4l2_subdev_core_ops dw9807_core_ops = {
	.ioctl = dw9807_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl32 = dw9807_compat_ioctl32,
#endif
};
...
static int dw9807_probe(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "dw9807-vcm probing at I2C addr 0x%02x\n",
		 client->addr);

	/* Verify device responds on I2C: read STATUS register */
	rval = dw9807_i2c_check(client);
	if (rval < 0) {
		dev_err(&client->dev,
			"dw9807-vcm not found or I2C error (ret=%d)\n", rval);
		return -ENODEV;
	}
	dev_info(&client->dev, "dw9807-vcm STATUS=0x%02x (0=ready)\n", rval);

	/* Default full-range move time; overridable from DT */
	dw9807_dev->vcm_movefull_t = 200;
	of_property_read_u32(client->dev.of_node, "rockchip,vcm-move-time-ms",
			     &dw9807_dev->vcm_movefull_t);

	v4l2_i2c_subdev_init(&dw9807_dev->sd, client, &dw9807_ops);
	dw9807_dev->sd.flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
	dw9807_dev->sd.internal_ops = &dw9807_int_ops;

	/* Rockchip module index/facing for subdev naming (used by rkaiq) */
	if (!of_property_read_u32(client->dev.of_node,
		...
		facing[0] = (strcmp(dw9807_dev->module_facing, "back") == 0) ?
			     'b' : 'f';
	...
	dw9807_dev->start_move_tv = ns_to_kernel_old_timeval(ktime_get_ns());
	dw9807_dev->end_move_tv = dw9807_dev->start_move_tv;

	dev_info(&client->dev, "dw9807-vcm probe success, subdev: %s\n",
		 dw9807_dev->sd.name);
	return 0;
...
err_cleanup:
	...
	dev_err(&client->dev, "dw9807-vcm probe failed: %d\n", rval);
	return rval;
}
